...
func TestReconcileAPI_DefaultStatusProbes(t *testing.T) {
	...
	for name, p := range map[string]*corev1.Probe{
		...
		if p == nil || p.HTTPGet == nil || p.HTTPGet.Path != apiStatusPath {
			t.Fatalf("%s probe path=%v want %s", name, p, apiStatusPath)
		}
...
func TestReconcileAPI_PodSpecOverlay(t *testing.T) {
	...
	if c0.ReadinessProbe == nil || c0.ReadinessProbe.HTTPGet == nil || c0.ReadinessProbe.HTTPGet.Path != apiStatusPath {
		t.Fatalf("default readiness expected, got %v", c0.ReadinessProbe)
	}
	if c0.StartupProbe == nil || c0.StartupProbe.HTTPGet == nil || c0.StartupProbe.HTTPGet.Path != apiStatusPath {
